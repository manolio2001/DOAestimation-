...
int main()
{
    ...
    vector<int> hist_bins;
    for (int i = 1; i <= 359; i++)
    {
        hist_bins.push_back(i);
    }

    // Preparation of the arrray for the results
    ...
    vector<double> All_estimates;

    for (int i = 0; i < EstimatesFrames.size(); i++)
    {
        for (int j = 0; j < EstimatesFrames[i].size(); j++)
        {
            if (!isnan(EstimatesFrames[i][j]))
            {
                All_estimates.push_back(EstimatesFrames[i][j]);
            }
        }
    }

    map<int, int> histogram;
    for(int bin = 0; bin < hist_bins.size(); bin++){
        histogram[bin] = 0;
    }
   

   for(int i = 0; i < All_estimates.size(); i++){
    int bin = static_cast<int>(All_estimates[i]);
    ...
    for (auto it = histogram.begin(); it != histogram.end(); ++it) {
        if (it->second > 0) {
            std::cout << "Angle: " << it->first << " Count: " << it->second << std::endl;
        }
    }
